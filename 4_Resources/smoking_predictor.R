...
main <- function()
{
	...
	SD <- as.numeric(arguments[3])
	
	#methylationfile <- '/lustre/home/sww208/USProject/3_ProcessedData/FormatedMethPhen2020.RData'
	#out_file <- '/lustre/home/sww208/USProject/4_Results/SmokingPred2020'
	#SD <- 5
	...
	m <- match(rownames(datMeth), pData[,"id"])
	pData <- pData[m,]
	
	message("Predicting smoking levels")
	datMeth <- as.data.frame(t(datMeth))
	smok <- predict.smoking(Illig_data, datMeth)
	...
	plot(pData$confage, smok$Smoking, xlab="Age", ylab="predicted smoking",main="Age vs Smoking prediction",cex.main=0.7)
